    ...
    def draw_bboxes(self,video_frames, player_detections):
        ...
        for frame, ball_dict in zip(video_frames, player_detections):
            ...
            for track_id, bbox in ball_dict.items():
                x1, y1, x2, y2 = bbox
                cv2.putText(frame, f"Ball ID: {track_id}",(int(bbox[0]),int(bbox[1] -10 )),cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 225), 2)
                cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (225,0,225), 2)
